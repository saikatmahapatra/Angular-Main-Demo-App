...
export class AddEditEmergencyContactComponent implements OnInit {
  ...
  getFormData() {
    this.apiSvc.get(MyAppConfig.apiUrl.emergencyContactFormData).subscribe((val: any) => {
  ...
  onSubmit() {
    ...
    if (this.myForm.valid && this.myForm.get('action')?.value === 'add') {
      this.apiSvc.post(MyAppConfig.apiUrl.addEmergencyContact, this.myForm.value).subscribe({
        next: (response: any) => {
      ...
      this.apiSvc.put(MyAppConfig.apiUrl.updateEmergencyContact, this.myForm.value).subscribe({
        next: (response: any) => {
          if (response.status == 'success') {
            this.alertSvc.setAlert('success', response.message, true);
            this.myForm.reset();
  ...
  getContact() {
    ...
    this.apiSvc.get(MyAppConfig.apiUrl.getEmergencyContact, options).subscribe((val: any) => {
