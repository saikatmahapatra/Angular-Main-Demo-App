...
export class AddEditExperienceComponent implements OnInit {
  ...
  get employerOptionsWithOthers() {
    return [...(this.employers || []), { id: '-1', name: 'Others' }];
  }

  get designationOptionsWithOthers() {
    return [...(this.designations || []), { id: '-1', name: 'Others' }];
  }
  ...
  getFormData() {
    this.apiSvc.get(MyAppConfig.apiUrl.experienceFormData).subscribe((val: any) => {
  ...
  getWorkExperience() {
    ...
    this.apiSvc.get(MyAppConfig.apiUrl.getExperience, options).subscribe((val: any) => {
